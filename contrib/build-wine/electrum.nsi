...
;Variables
  ...
  !define PRODUCT_NAME "electrum-FTO"
  !define PRODUCT_WEB_SITE "https://github.com/futuro-coin/electrum"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-FTO-3.1.2-setup-win32.exe"
...
Section
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-FTO.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-FTO.exe" "" "$INSTDIR\electrum-FTO.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-FTO.exe" "--testnet" "$INSTDIR\electrum-FTO.exe" 0


  ;Links futurocoin: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\futurocoin" "" "URL:futurocoin Protocol"
  WriteRegStr HKCU "Software\Classes\futurocoin" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\futurocoin" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\futurocoin\shell\open\command" "" "$\"$INSTDIR\electrum-FTO.exe$\" $\"%1$\""
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\futurocoin"
